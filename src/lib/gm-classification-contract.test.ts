...
test("the flag NEVER enters a metric path — display and reporting only", () => {
  ...
  // side-by-side reporting (store-attendance.ts) is the ONE sanctioned
  // metric-adjacent consumer, and it persists nothing.
  const METRIC_PATHS = [
...
test("store-wide reporting shows BOTH ways from summed parts — never averaged, never one number", () => {
